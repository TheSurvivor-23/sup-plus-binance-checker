...
const SUPPORT_HANDLE = Deno.env.get("SUPPORT_HANDLE") || "@SPlusX_Help";
...
const TX = {
  ...
  send_txid: { en: "Now send your TXID / Transaction Hash in this chat.", ar: "الآن أرسلي TXID / Transaction Hash في الشات.", vi: "Bây giờ hãy gửi TXID / Transaction Hash trong cuộc trò chuyện này.", hi: "अब इसी चैट में TXID / Transaction Hash भेजें.", ur: "اب اسی چیٹ میں TXID / Transaction Hash بھیجیں۔" },
  valid_txid: { en: "Send a valid TXID / Transaction Hash starting with 0x.", ar: "أرسلي TXID صحيح يبدأ بـ 0x.", vi: "Gửi TXID / Transaction Hash hợp lệ bắt đầu bằng 0x.", hi: "0x से शुरू होने वाला सही TXID / Transaction Hash भेजें.", ur: "0x سے شروع ہونے والا درست TXID / Transaction Hash بھیجیں۔" },
...
async function markPaidPrompt(chatId, orderId, previousMessageId = null) {
  ...
  await storeSet(["active_order", String(chatId)], { ...order, status: "WAITING_TXID", lang, updated_at: nowIso() });
...
async function handleTxid(chatId, text, from) {
  ...
  if (!/^0x[a-fA-F0-9]{8,}$/.test(txid)) return await sendMessage(chatId, `⚠️ ${t(lang, "valid_txid")}`);
...
Expected: ${money(result.expectedAmount)} USDT`);
  if (result.status === "CONFIG_ERROR") return await sendMessage(chatId, `⚠️ ${t(lang, "checker_config")}`);
  return await sendMessage(chatId, `⚠️ ${t(lang, "payment_not_found")}`);
}
...
function isAdminCommand(text) {
  return text === "/admin" || text === "/stock" || text === "/orders" || text === "/pending" || text === "/users" || text === "/stats" || text === "/adminhelp" || /^\/(setstock|addstock|price|hide|show|image|setname|setdesc|mode|additem|items|order|deliver|addproduct|broadcast)\s/.test(text);
}
