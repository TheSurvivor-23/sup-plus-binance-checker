...
async function showProductsPage(chatId, page = 1, messageId = null) {
  ...
  rows.push([
    ...
    { text: "Sort: All", callback_data: "noop" },
  ]);
  rows.push([
...
✅ Binance Deposit is available.

For normal product orders, customers can still pay directly during checkout.`;
...
After sending, copy the TXID / Transaction Hash and use it in your order payment step.`;
  return await sendCard(chatId, text, { inline_keyboard: [[{ text: "🏠 Home", callback_data: "menu" }]] }, "", previousMessageId);
}
async function showProfile(chatId, previousMessageId = null) {
  const text = `👤 Profile

💰 Balance: $0.00 USDT
🧾 Orders: use My Orders

Need help? ${SUPPORT_HANDLE}`;
  return await sendCard(chatId, text, { inline_keyboard: [
    [{ text: "🧾 My Orders", callback_data: "orders_menu" }],
    [{ text: "💳 Top Up", callback_data: "topup" }],
    [{ text: "🏠 Home", callback_data: "menu" }],
  ] }, "", previousMessageId);
}
