...
function classifyPaymentRef(ref) {
  ...
  // TXID / Transaction Hash: this is the ONLY reference that can be verified automatically
  // through Binance Deposit History. It must be a real long 0x hash.
  if (/^0x[a-fA-F0-9]{32,}$/.test(s)) return "TXID";

  // Binance Pay Order ID: keep this option, but it is MANUAL REVIEW only unless you add
  // Binance Pay Merchant API later. Short random numbers are rejected.
  if (/^[0-9]{16,30}$/.test(s)) return "BINANCE_PAY_ORDER_ID";

  // CXID: must clearly start with CX or CXID. Random words/numbers are rejected.
  if (/^(CX|CXID)[-_]?[A-Za-z0-9]{6,40}$/i.test(s)) return "CXID";
...
${refType === "TXID" ? "TXID auto-check did not confirm payment. Do NOT approve unless you verify manually in Binance." : "Review Binance Pay history, then approve or reject."}`, approveKeyboard(order.order_id));
}

async function handleTopupReference(chatId, text, from) {
  const topup = await storeGet(["active_topup", String(chatId)]);
  if (!topup || !["WAITING_PAYMENT_REF", "PAYMENT_REJECTED", "CHECKING_TXID", "WAITING_ADMIN_APPROVAL"].includes(topup.status)) return false;
  ...
  if (refType === "INVALID") {
    return await sendMessage(chatId, `❌ Invalid payment reference.

Accepted formats:
• TXID / Transaction Hash: must start with 0x and be long
• Binance Pay Order ID: 16-30 digits, admin review only
• CXID: must start with CX or CXID, admin review only

الأرقام القصيرة أو العشوائية لا يتم قبولها.`);
  }
...
Previous order/top up: ${used.order_id || "unknown"}`);
  ...
  if (refType === "TXID") {
    ...
    if (result.paid) {
      await storeSet(["payment_refs", refType, normalizePaymentRef(ref)], { order_id: topup.topup_id, chat_id: String(chatId), ref, ref_type: refType, used_at: nowIso(), type: "topup", status: "PAID" });
...
TXID: ${ref}`);
    ...
    topup.status = "WAITING_PAYMENT_REF";
    topup.binance_check = result;
    topup.updated_at = nowIso();
    await storeSet(["topups", topup.topup_id], topup);
    await storeSet(["active_topup", String(chatId)], topup);

    if (result.status === "AMOUNT_MISMATCH") {
      return await sendMessage(chatId, `⚠️ Top up payment found, but amount is wrong.

Received: ${money(result.amount)} USDT
Expected: ${money(result.expectedAmount)} USDT

Please send the exact amount shown. No refund for wrong amount/network.`);
    }
    if (result.status === "CONFIG_ERROR") {
      if (ADMIN_ID) await sendMessage(ADMIN_ID, `⚠️ Binance checker config error for top up ${topup.topup_id}`);
      return await sendMessage(chatId, `⚠️ Payment checker is not configured. Admin was notified.

Top Up ID: ${topup.topup_id}`);
    }
    return await sendMessage(chatId, `❌ Payment not found on Binance.

We checked your TXID and did not find a successful USDT deposit with the exact amount.

Please check:
• TXID / Transaction Hash is correct
• Network is correct
• Final received amount equals ${money(topup.amount)} USDT
• Wait 2-5 minutes if the transfer is new

Top Up ID: ${topup.topup_id}`);
  }

  // Binance Pay Order ID / CXID cannot be checked with normal Binance Deposit API.
  // Keep it available, but wait for admin review.
  await storeSet(["payment_refs", refType, normalizePaymentRef(ref)], { order_id: topup.topup_id, chat_id: String(chatId), ref, ref_type: refType, used_at: nowIso(), type: "topup", status: "WAITING_ADMIN_APPROVAL" });
  await sendMessage(chatId, `🟡 Top up reference received, but payment is NOT confirmed yet.

Binance Pay Order ID / CXID requires admin review.
...
Review Binance Pay history, then approve or reject.`, { inline_keyboard: [[{ text: "✅ Approve top up", callback_data: `admin_approve_topup_${topup.topup_id}` }],[{ text: "❌ Reject top up", callback_data: `admin_reject_topup_${topup.topup_id}` }]] });
  return true;
}
...
async function handlePaymentReference(chatId, text, from) {
  ...
  if (!order || !["WAITING_PAYMENT_REF", "WAITING_TXID", "PAYMENT_REJECTED", "CHECKING_TXID", "WAITING_ADMIN_APPROVAL"].includes(order.status)) return false;
  ...
  if (refType === "INVALID") {
    return await sendMessage(chatId, `⚠️ ${t(lang, "valid_txid")}

Accepted formats:
• TXID / Transaction Hash: starts with 0x and is long
• Binance Pay Order ID: 16-30 digits, admin review only
• CXID: starts with CX, admin review only
...
Delivery: ${delivery.adminText}`);
    ...
    order.status = "WAITING_PAYMENT_REF";
    ...
    if (result.status === "AMOUNT_MISMATCH") {
      return await sendMessage(chatId, `⚠️ ${t(lang, "amount_wrong")}.
${btn(lang, "amount")}: ${money(result.amount)} USDT
Expected: ${money(result.expectedAmount)} USDT

⚠️ Send the exact amount shown. No refund for wrong amount or wrong network.`);
    }
    if (result.status === "CONFIG_ERROR") {
      if (ADMIN_ID) await sendMessage(ADMIN_ID, `⚠️ Binance checker config error for order ${order.order_id}`);
      return await sendMessage(chatId, `⚠️ ${t(lang, "checker_config")}

Order: ${order.order_id}`);
    }

    return await sendMessage(chatId, `❌ ${t(lang, "payment_not_found")}

We checked Binance and did NOT find a successful USDT deposit matching this TXID and exact amount.

Order: ${order.order_id}
Expected amount: ${money(order.amount)} USDT

⚠️ Send the exact amount shown.
⚠️ If Binance charges a fee, make sure the final received amount equals the order amount.
⚠️ Wrong amount or wrong network cannot be refunded.`);
  }

  // Binance Pay Order ID / CXID remains available, but it is not auto-confirmed.
  // It only goes to admin review and requires manual approval.
  await claimPaymentRef(order, ref, refType);
  await sendMessage(chatId, `🟡 Payment reference received, but payment is NOT confirmed yet.

Your Binance Pay Order ID / CXID is waiting for admin review.

Order: ${order.order_id}

━━━━━━━━━━━━━━

🟡 تم استلام رقم الدفع، لكن الدفع لم يتم تأكيده بعد.

رقم Binance Pay Order ID / CXID بانتظار مراجعة الأدمن.
