...
async function showHome(chatId) {
  ...
  return await safeSend(chatId, text, homeKeyboard(), STORE_IMAGE_URL);
}
...
function productDetailsText(p) {
  ...
  const warranty = p.warranty === "YES" ? `Yes · ${p.warranty_days || 30} days` : "No warranty";
  return `🔮 ${p.name_en || p.button_name}\n\n💵 Price: $${money(p.price)} / code\n📦 Stock: ${p.stock}\n${inStock ? "🟢 Available" : "🔴 Out of stock"}\n🚚 Delivery: ${p.delivery_mode === "auto" ? "Automatic after payment" : "Manual after payment"}\n🛡 Warranty: ${warranty}\n\n━━━━━━━━━━━━━━\n${p.description_en || p.description_ar || "Digital product subscription."}\n\nProduct ID: ${p.product_id}\nDuration: ${p.duration}\n\n${p.delivery_mode === "auto" ? "Delivery is automatic after payment confirmation." : "After payment confirmation, admin will process delivery."}`;
}
...
async function showProduct(chatId, productId) {
  ...
  return await safeSend(chatId, productDetailsText(p), productKeyboard(p), p.image_url);
}
...
async function showQuantity(chatId, productId) {
  const p = await findProduct(productId);
  ...
  return await safeSend(chatId, text, quantityKeyboard(p), p.image_url);
}
...
async function showSummary(chatId, data) {
  ...
  return await safeSend(chatId, text, markup, p.image_url);
}
...
async function showPayment(chatId, from, data) {
...
async function markPaidPrompt(chatId, orderId) {
...
function isAdminCommand(text) {
  return text === "/admin" || text === "/stock" || text === "/orders" || text === "/stats" || text === "/adminhelp" || /^\/(setstock|addstock|price|hide|show|image|mode|additem|items|addproduct|broadcast)\s/.test(text);
}
...
async function handleMessage(message) {
  ...
  if (text === "/support") return await sendMessage(chatId, `🎧 Support\n${SUPPORT_HANDLE}`, homeKeyboard());
...
async function handleCallback(q) {
  ...
  if (data === "support") return await sendMessage(chatId, `🎧 Support\n${SUPPORT_HANDLE}`, homeKeyboard());
  if (data.startsWith("products_p")) return await showProductsPage(chatId, Number(data.replace("products_p", "")), messageId);
  if (data.startsWith("product_")) return await showProduct(chatId, data.replace("product_", ""));
  if (data.startsWith("buy_")) return await showQuantity(chatId, data.replace("buy_", ""));
  if (data.startsWith("qty_")) return await showSummary(chatId, data);
  if (data.startsWith("confirm_")) return await showPayment(chatId, q.from, data);
  if (data.startsWith("paid_")) return await markPaidPrompt(chatId, data.replace("paid_", ""));
  if (data === "cancel_order") return await cancelOrder(chatId);
